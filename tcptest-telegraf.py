...
class NetworkTester:
  def __init__(self, iperfServer, iperfServerPort, iperfServerUploadMbits, iperfServerUploadDuration, server, iperfUsername, iperfPassword, iperfPublicKeyFile, iperfMode):
  ...
  def calliPerf(self):
    ...
    environment["IPERF3_PASSWORD"] = self.iperfPassword
    iperfArguments = [
        'iperf3',
        '-c', self.iperfServer,
  ...
  def runiPerfTest(self):
    ...
    try:
      ...
      tagName = "{}_mbps".format(self.iperfMode)
      metric.add_value(tagName, float(summary['bits_per_second']))
    finally:
...
parser.add_argument('--iperf-mode', default="download", help = 'download or upload')
parser.add_argument('--server', default = socket.gethostname(), help='Name of server doing measurement')

args = parser.parse_args()
tester = NetworkTester(args.iperf_server, args.iperf_server_port, args.iperf_upload_mbits, args.iperf_upload_duration, args.server, args.iperf_username, args.iperf_password, args.iperf_public_key_file, args.iperf_mode)
